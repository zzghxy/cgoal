...
class CParser:
    ...
    def _parse_external_declaration(self) -> List[c_ast.Node]:
        ...
        if tok.type == "COMMENT":
            ...
            return [c_ast.Comment(comment.value, self._tok_coord(comment))]
        if tok.type == "PPHASH":
        ...
        # CGoal: let-declaration
        if tok.type == "LET":
            return self._parse_let_declaration()

        # CGoal: class-definition
        if tok.type == "CLASS":
            return [self._parse_class_def()]

        # CGoal: namespace-definition
        if tok.type == "NAMESPACE":
            return [self._parse_namespace()]

        # CGoal: using-directive / using-declaration
        if tok.type == "USING":
            return self._parse_using_directive()

        # CGoal: goal-definition
        if tok.type == "GOAL":
            return [self._parse_goal_block()]

        # CGoal: step-definition (top-level)
        if tok.type == "STEP":
            return [self._parse_step_block()]

        # CGoal: impl-definition
        if tok.type == "IMPL":
            return [self._parse_impl_block()]

        # CGoal: alias-definition
        if tok.type == "ALIAS":
            return [self._parse_alias_def()]

        # CGoal: extern "C" block
        if tok.type == "EXTERN" and self._peek_type(2) in _STRING_LITERAL:
            return [self._parse_extern_c_block()]
    ...
    def _parse_statement(self) -> c_ast.Node | List[c_ast.Node]:
        ...
        match tok_type:
            ...
            case "ABORT" | "FINISH" | "SKIP":
                return self._parse_cgoal_jump_statement()
    ...
    # BNF: block_item : declaration | statement | let_declaration | step_definition
    def _parse_block_item(self) -> c_ast.Node | List[c_ast.Node]:
        if self._peek_type() == "LET":
            return self._parse_let_declaration()
        if self._peek_type() == "STEP":
            return self._parse_step_block()
        if self._starts_declaration():
            ...
            case "IF":
                if self._accept("CONSTEXPR"):
                    ...
                    if self._accept("ELSE"):
                        ...
                        return c_ast.If(cond, then_stmt, else_stmt,
                                        self._tok_coord(tok))
                    return c_ast.If(cond, then_stmt, None,
                                    self._tok_coord(tok))
                self._expect("LPAREN")
                cond = self._parse_expression()
                self._expect("RPAREN")
    ...
    def _parse_expression(self) -> c_ast.Node:
        expr = self._parse_pipe_expression()
    ...
    def _parse_unary_expression(self) -> c_ast.Node:
        ...
        if tok_type == "AT":
            tok = self._advance()
            name_tok = self._expect("ID")
            return c_ast.NamedData(
                name_tok.value, self._tok_coord(tok)
            )
    ...
    def _parse_primary_expression(self) -> c_ast.Node:
        tok_type = self._peek_type()
        if tok_type in {"ID", "TYPEID"}:
            tok = self._advance()
            expr: c_ast.Node = c_ast.ID(tok.value, self._tok_coord(tok))
            while self._accept("DCOLON"):
                member_tok = self._expect("ID")
                expr = c_ast.StructRef(
                    expr, "::",
                    c_ast.ID(member_tok.value, self._tok_coord(member_tok)),
                    self._tok_coord(tok),
                )
            return expr
        if (
    ...
    # ------------------------------------------------------------------
    # CGoal Parsing Methods
    # ------------------------------------------------------------------

    def _parse_let_declaration(self) -> List[c_ast.Node]:
        let_tok = self._expect("LET")
        name_tok = self._expect("ID")
        ...
        return [
            c_ast.LetDecl(
                ...
                type=type_node,
                init=init,
                coord=self._tok_coord(let_tok),
            )
        ]

    def _parse_class_def(self) -> c_ast.ClassDef:
        class_tok = self._expect("CLASS")
        name_tok = self._expect("ID")
        
        # 将类名添加为类型名，使得后续可以使用 let p: Person = ...
        self._add_typedef_name(name_tok.value, self._tok_coord(class_tok))
        ...
        while self._peek_type() not in {"RBRACE", None}:
            if self._peek_type() == "LET":
                decls.extend(self._parse_let_declaration())
            elif self._starts_declaration():
                decls.extend(self._parse_declaration())
            elif self._accept("SEMI"):
                continue
            else:
                self._parse_error("Expected declaration in class body",
                                  self._tok_coord(self._peek()))

        self._expect("RBRACE")

        return c_ast.ClassDef(
            name=name_tok.value,
            bases=[],
            decls=decls,
            coord=self._tok_coord(class_tok),
        )

    def _parse_namespace(self) -> c_ast.Namespace:
        ns_tok = self._expect("NAMESPACE")
        name_tok = self._expect("ID")
        self._expect("LBRACE")
        ...
        return c_ast.Namespace(
            name=name_tok.value,
            decls=decls,
            coord=self._tok_coord(ns_tok),
        )

    def _parse_using_directive(self) -> List[c_ast.Node]:
        using_tok = self._expect("USING")

        if self._peek_type() == "NAMESPACE":
            self._advance()
            name_tok = self._expect("ID")
            self._expect("SEMI")
            return [
                c_ast.UsingDirective(
                    name=name_tok.value,
                    namespace=True,
                    symbol=None,
                    coord=self._tok_coord(using_tok),
                )
            ]

        name_tok = self._expect("ID")
        self._expect("DCOLON")
        symbol_tok = self._expect("ID")
        self._expect("SEMI")
        ...
        return [
            c_ast.UsingDirective(
                name=name_tok.value,
                namespace=False,
                symbol=symbol_tok.value,
                coord=self._tok_coord(using_tok),
            )
        ]

    def _parse_goal_block(self) -> c_ast.GoalBlock:
        goal_tok = self._expect("GOAL")
        spec, saw_type, _ = self._parse_declaration_specifiers(allow_no_type=False)
        name_tok = self._expect("ID")
        self._expect("LPAREN")
        params = self._parse_parameter_type_list_opt()
        self._expect("RPAREN")
        ...
        if self._accept("SEMI"):
            return c_ast.GoalBlock(
                name=name_tok.value,
                inputs=params.params if params else [],
                outputs=spec["type"],
                steps=[],
                coord=self._tok_coord(goal_tok),
            )
    ...
    def _extract_steps(self, body: c_ast.Compound) -> List[c_ast.Node]:
        steps = []
        if body.block_items:
    ...
    def _parse_step_block(self) -> c_ast.StepBlock:
        step_tok = self._expect("STEP")
        spec, saw_type, _ = self._parse_declaration_specifiers(allow_no_type=False)
        name_tok = self._expect("ID")
        self._expect("LPAREN")
        params = self._parse_parameter_type_list_opt()
        ...
        return c_ast.StepBlock(
            name=name_tok.value,
            inputs=params.params if params else [],
            outputs=spec["type"],
            body=body,
            coord=self._tok_coord(step_tok),
        )

    def _parse_impl_block(self) -> c_ast.ImplBlock:
        impl_tok = self._expect("IMPL")
        name_tok = self._expect("ID")
        self._expect("USING")
        target_tok = self._expect("ID")
        body = self._parse_compound_statement()

        return c_ast.ImplBlock(
            name=name_tok.value,
            target=target_tok.value,
            body=body,
            coord=self._tok_coord(impl_tok),
        )

    def _parse_alias_def(self) -> List[c_ast.Node]:
        alias_tok = self._expect("ALIAS")
        name_tok = self._expect("ID")
        self._expect("EQUALS")
        str_tok = self._expect("STRING_LITERAL")
        self._expect("SEMI")
        ...
        return [
            c_ast.AliasDef(
                name=name_tok.value,
                value=str_tok.value,
                coord=self._tok_coord(alias_tok),
            )
        ]

    def _parse_extern_c_block(self) -> c_ast.ExternCBlock:
        extern_tok = self._expect("EXTERN")
        str_tok = self._expect("STRING_LITERAL")
        body = self._parse_compound_statement()

        return c_ast.ExternCBlock(
            linkage=str_tok.value,
            decls=body.block_items if body.block_items else [],
            coord=self._tok_coord(extern_tok),
        )

    def _parse_cgoal_jump_statement(self) -> c_ast.Node:
        tok = self._advance()
        match tok.type:
            case "ABORT":
                self._expect("LPAREN")
            ...
            case "SKIP":
                self._expect("SEMI")
                zero = c_ast.Constant("int", 0, self._tok_coord(tok))
                return c_ast.Return(zero, self._tok_coord(tok))
            case _:
                self._parse_error("Invalid CGoal jump statement",
                                  self._tok_coord(tok))
