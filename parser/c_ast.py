...
class UsingDirective(Node):
    __slots__ = ('name', 'namespace', 'symbol', 'coord', '__weakref__')
    def __init__(self, name, namespace, symbol, coord=None):
        self.name = name
    ...
    attr_names = ('name', 'namespace', 'symbol', )

class ClassDef(Node):
    __slots__ = ('name', 'bases', 'decls', 'coord', '__weakref__')
    def __init__(self, name, bases, decls, coord=None):
        self.name = name
        self.bases = bases
        self.decls = decls
        self.coord = coord
    ...
    def children(self):
        nodelist = []
        for i, child in enumerate(self.bases or []):
            nodelist.append((f"bases[{i}]", child))
        for i, child in enumerate(self.decls or []):
            nodelist.append((f"decls[{i}]", child))
        return tuple(nodelist)

    def __iter__(self):
        for child in (self.bases or []):
            yield child
        for child in (self.decls or []):
            yield child
...
class GoalBlock(Node):
    __slots__ = ('name', 'inputs', 'outputs', 'steps', 'coord', '__weakref__')
    def __init__(self, name, inputs, outputs, steps, coord=None):
        self.name = name
        self.inputs = inputs
    ...
    def children(self):
        ...
        for i, child in enumerate(self.inputs or []):
            nodelist.append((f"inputs[{i}]", child))
        for i, child in enumerate(self.outputs or []):
            nodelist.append((f"outputs[{i}]", child))
        for i, child in enumerate(self.steps or []):
            nodelist.append((f"steps[{i}]", child))
        return tuple(nodelist)

    def __iter__(self):
        for child in (self.inputs or []):
            yield child
        for child in (self.outputs or []):
            yield child
        for child in (self.steps or []):
            yield child

    attr_names = ('name', )

class StepBlock(Node):
    __slots__ = ('name', 'inputs', 'outputs', 'body', 'coord', '__weakref__')
    def __init__(self, name, inputs, outputs, body, coord=None):
        self.name = name
        self.inputs = inputs
        self.outputs = outputs
        self.body = body
    ...
    def children(self):
        ...
        if self.inputs is not None:
            nodelist.append(("inputs", self.inputs))
        if self.outputs is not None:
            nodelist.append(("outputs", self.outputs))
        if self.body is not None:
            nodelist.append(("body", self.body))
        return tuple(nodelist)
...
class ImplBlock(Node):
    __slots__ = ('name', 'target', 'body', 'coord', '__weakref__')
    def __init__(self, name, target, body, coord=None):
        self.name = name
        self.target = target
        self.body = body
        self.coord = coord
...
class ExternCBlock(Node):
    __slots__ = ('linkage', 'decls', 'coord', '__weakref__')
    def __init__(self, linkage, decls, coord=None):
